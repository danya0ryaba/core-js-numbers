...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multiVector = x1 * x2 + y2 * y1;
  const moduleX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const moduleY = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAngle = multiVector / (moduleX * moduleY);
 ...
 */
// убрать
// const stringArrayValue = String(value).split('');
// return stringArrayValue[stringArrayValue.length - 1];
...
function getFibonacciNumber(index) {
  ...
  for (let i = index; i >= 0; i -= 1) {
    arrayFibonacci.push(step);
    const addStep = arrayFibonacci[arrayFibonacci.length - 2] || 0;
    step = arrayFibonacci[arrayFibonacci.length - 1] + addStep;
  }
